...
RRT::RRT(): rclcpp::Node("rrt_node"), gen((std::random_device())()) {
    ...
    string file = "/home/student/sim_ws/src/race_2_rrt/race2_pts.csv";
    initWaypoints(file);
...
void RRT::publishDriveMsg(vector<double> goal){
    ...
    double speed = 4.0;
    float video_mult = 1.0; //11.0 / 39;
    speed *= video_mult;

    auto drive_msg = ackermann_msgs::msg::AckermannDriveStamped();
    drive_msg.drive.steering_angle = angleControl;
    if(abs(angleControl) > 20.0 / 180.0 * (22/7))
        drive_msg.drive.speed = float(0.5 * speed);
    else if(abs(angleControl) > 10.0 / 180.0 * (22/7))
        drive_msg.drive.speed = float(0.75 * speed);
    else
        drive_msg.drive.speed = float(1.0 * speed);
